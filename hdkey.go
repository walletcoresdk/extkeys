...
var (
	...
	MainnetVersion byte = 0x80 // 主网版本字节
	TestnetVersion byte = 0xEF // 测试网版本字节
	...
	// BIP 173.
	Bech32HRPSegwit string = "bc" // always bc for main net
...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	child := &ExtendedKey{
		...
		FingerPrint: Hash160(k.pubKeyBytes())[:4],
	}
...
func (k *ExtendedKey) BIP44Child(coinType, i uint32) (*ExtendedKey, error) {
	...
	return k.SubBIP44Child(coinType, i)
}

func (k *ExtendedKey) SubBIP44Child(coinType, i uint32) (*ExtendedKey, error) {
	if !k.IsPrivate {
	...
	extKey, err := k.Derive(append([]uint32{
		HardenedKeyStart + 44,       // purpose
...
// ToECDSA returns the key data as ecdsa.PrivateKey
func (k *ExtendedKey) ToECDSABTC() *btcec.PrivateKey {
	privKey, _ := btcec.PrivKeyFromBytes(btcec.S256(), k.KeyData)
	return privKey
}
